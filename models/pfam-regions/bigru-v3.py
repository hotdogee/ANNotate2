...
def model_fn(features, labels, mode, params, config):
    ...
    with tf.variable_scope('conv_1'):
        if params.use_conv_batch_norm:
            ...
            # convolved_norm = tf.contrib.layers.batch_norm(
            #     inputs=convolved,
            #     decay=0.999,
            #     center=True,
            #     scale=False,
            ...
            #     adjustment=None
            # )
            convolved_norm_activation = tf.nn.relu(
                convolved_norm,
                name='relu'
            )
            dropped_convolved = tf.layers.dropout(
                inputs=convolved_norm_activation,
    ...
    with tf.variable_scope('bi_rnn_1'):
        ...
        if params.use_cudnn:
            ...
            outputs, output_state_fw, output_state_bw = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(
                cells_fw=[tf.contrib.rnn.AttentionCellWrapper(cell(params.rnn_num_units), 2)], # 32
                cells_bw=[tf.contrib.rnn.AttentionCellWrapper(cell(params.rnn_num_units), 2)],
    ...
    with tf.variable_scope('attention_1'):
        if params.use_rnn_attention:
            attention_size = params.attention_size # 50

            if isinstance(outputs, tuple):
                # In case of Bi-RNN, concatenate the forward and the backward RNN outputs.
                outputs = tf.concat(outputs, 2)

            hidden_size = outputs.shape[2].value  # D value - hidden size of the RNN layer
            ...
            outputs = outputs * tf.expand_dims(alphas, -1)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help='The size of an attention vector.')
