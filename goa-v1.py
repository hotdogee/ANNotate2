...
def parse_example(serialized, mode):
...
def model_fn(features, labels, mode, params, config):
    ...
    if params.use_output_highway: # True
        ...
        for d in range(params.output_highway_depth): # 3
            with tf.variable_scope('highway_{}'.format(d)):
                H = tf.layers.dense(
                    inputs=outputs,
                    units=params.output_highway_units,
                    activation=tf.nn.relu,
                    use_bias=True,
                    kernel_initializer=None,
                    bias_initializer=tf.zeros_initializer(),
    ...
    with tf.variable_scope('output_1'):
        ...
        # logits shape=(batch_size, num_classes), dtype=float32
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        with tf.variable_scope('loss'):
            if params.use_crf:
                ...
                loss = tf.reduce_sum(losses) / tf.cast(batch_size, tf.float32)
