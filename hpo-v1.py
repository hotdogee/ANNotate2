...
def parse_example_go_v1(serialized, mode):
    """Parse a single v2 record which is expected to be a tensorflow.Example."""
    if mode != tf.estimator.ModeKeys.PREDICT:
        features = {
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        ...
        # tf.Tensor: shape=(go_total,), dtype=uint8
        go = tf.cast(go, tf.float32)
...
def parse_example_go_v2(serialized, mode):
    """Parse a single v2 record which is expected to be a tensorflow.Example."""
    if mode != tf.estimator.ModeKeys.PREDICT:
        features = {
            'protein': tf.FixedLenFeature(shape=(), dtype=tf.string),
...
def parse_example_hpo_seq2seq(serialized, mode):
    """Parse a single hpo v1 seq2seq tfrecord."""
    if mode != tf.estimator.ModeKeys.PREDICT:
        features = {
            'protein': tf.FixedLenFeature(shape=(), dtype=tf.string),
            'hpo': tf.FixedLenFeature(shape=(), dtype=tf.string)
        }
    else:
        features = {'protein': tf.FixedLenFeature(shape=(), dtype=tf.string)}
    parsed = tf.parse_single_example(
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        target = tf.decode_raw(
            input_bytes=parsed['hpo'],
            out_type=tf.uint16,
            little_endian=True,
            name=None
        )
        # tf.Tensor: shape=(hpo_total,), dtype=uint16
        target = tf.cast(x=target, dtype=tf.int32, name=None)
        # tf.Tensor: shape=(hpo_total,), dtype=int32
        lengths = tf.shape(input=target, name=None, out_type=tf.int32)[0]
        del parsed['hpo']
        label = {'target': target, 'lengths': lengths}
        # domains = tf.one_hot(domains, params.num_classes)
        return parsed, label
    else:
        return parsed
...
def input_fn(mode, params, config):
    ...
    parse_fn = parse_example_hpo_seq2seq
    dataset = dataset.map(
    ...
    dataset = dataset.apply(
        tf.data.experimental.bucket_by_sequence_length(
            ...
            # bucket_boundaries=[2**x for x in range(4, 14)],  # 16 ~ 8192
            # bucket_batch_sizes=[params.batch_size * 2**x for x in range(10, -1, -1)],  # 1024 ~ 1
            # bucket_boundaries=[2**x for x in range(5, 14)],  # 32 ~ 8192
            bucket_batch_sizes=[min(params.batch_size * 2**x, params.max_batch_size) for x in range(10, -1, -1)],  # [512] + [512 ~ 1]
...
def model_fn(features, labels, mode, params, config):
    ...
    target_sequence_lengths = labels['lengths']
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        seq_total = params.metadata['train']['accessions']
    elif mode == tf.estimator.ModeKeys.EVAL:
        seq_total = params.metadata['test']['accessions']
    ...
    with tf.name_scope("padding"):
        ...
        target_mask = tf.cast(tf.sign(target), dtype=tf.float32)  # 0 = 'PAD'
    ...
    #     # label_prob = tf.gather(all_probs, tf.expand_dims(labels, -1), batch_dims=-1)
    #     label_prob = tf.batch_gather(all_probs, tf.expand_dims(labels, -1))
...
def main(unused_args):
    ...
    if not FLAGS.num_classes or FLAGS.num_classes < 1:
        FLAGS.num_classes = len(FLAGS.metadata['hpo_list']) # 7793
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='v1',
        help=
        'Data format version of training and evaluation data'
    )
    parser.add_argument(
        '--label_type',
        type=str,
    ...
    parser.add_argument(
        ...
        default=1024,
        help=
        'Max batch size for short sequences'
    )
    parser.add_argument(
        '--prefetch_buffer',
        type=int,
