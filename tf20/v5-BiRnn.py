...
def pad_to_multiples(features, labels, pad_to_mutiples_of=8, padding_values=0):
    ...
    """
    max_len = tf.shape(input=labels)[1]
    target_len = tf.cast(tf.multiply(tf.math.ceil(tf.truediv(max_len, pad_to_mutiples_of)), pad_to_mutiples_of), tf.int32)
...
def bucket_by_sequence_length_and_pad_to_multiples(element_length_func,
    ...
    """
    with tf.compat.v1.name_scope("bucket_by_sequence_length_and_pad_to_multiples"):
        ...
        def batching_fn(bucket_id, grouped_dataset):
            ...
            if pad_to_bucket_boundary:
                ...
                check = tf.compat.v1.assert_less(
                    bucket_id,
...
def parse_sequence_example(serialized, mode):
    ...
    sequence['protein'] = tf.io.decode_raw(
        input_bytes=sequence['protein'],
        out_type=tf.uint8,
        little_endian=True,
        name=None
    )
...
# keys_tensor = tf.constant([x for x in aa_list])
# vals_tensor = tf.constant([x for x in range(len(aa_list))])
...
def serving_input_str_receiver_fn():
    ...
    # mode = tf.estimator.ModeKeys.PREDICT
    dataset = tf.data.Dataset.from_tensor_slices(serialized)
    # table.lookup(tf.constant([x for x in aa_list])
    # x = tf.constant('FIV')
    dataset = dataset.map(lambda x: table.lookup(tf.compat.v1.string_split([x], delimiter="").values))
...
class EpochCheckpointInputPipelineHookSaver(tf.compat.v1.train.Saver):
...
class EpochCheckpointInputPipelineHook(tf.estimator.SessionRunHook):
    ...
    def __init__(self,
        ...
        tf.compat.v1.logging.info("Create EpochCheckpointInputPipelineHook.")
    ...
    def begin(self):
        ...
        self._summary_writer = tf.compat.v1.summary.FileWriterCache.get(self._checkpoint_dir)
    ...
    def end(self, session):
        ...
        for f in input_checkpoint_files:
            if f.exists():
                f.unlink()
                # print('DELETE: ', f)
        tf.compat.v1.logging.debug("Removed input checkpoints")
    ...
    def _save(self, session, step):
        ...
        tf.compat.v1.logging.info("Saving\033[31m input\033[0m checkpoints for %d into %s.", step, self._save_path)
...
class EpochCheckpointSaverHook(tf.estimator.CheckpointSaverHook):
    ...
    * estimator.train or tf.estimator.train_and_evaluate will exit after the OutOfRangeError,
    ...
    def __init__(self,
        ...
        ValueError: At most one of saver or scaffold should be set.
        """
        tf.compat.v1.logging.info("Create EpochCheckpointSaverHook.")
    ...
    def begin(self):
        ...
        if self._epoch_saver is None:
            self._epoch_saver = tf.compat.v1.train.Saver(
                sharded=False,
                max_to_keep=9999,
    ...
    def _save_epoch(self, session, step, epoch):
        ...
        tf.compat.v1.logging.info("Saving\033[1;31m epoch\033[0m checkpoints for %d into %s.", step, save_path)
    ...
    def _save_step(self, session, step):
        """Saves the latest checkpoint, returns should_stop."""
        tf.compat.v1.logging.info("Saving\033[1;31m step\033[0m checkpoints for %d into %s.", step, self._step_save_path)
...
class EpochProgressBarHook(tf.estimator.SessionRunHook):
    def __init__(self,
                 total,
                 initial_tensor,
                 n_tensor,
    ...
    def before_run(self, run_context):  # pylint: disable=unused-argument
        ...
        None or a `SessionRunArgs` object.
        """
        return tf.estimator.SessionRunArgs(self._n_tensor)
...
class EvalProgressBarHook(tf.estimator.SessionRunHook):
    def __init__(self,
                 total,
                 n_tensor,
                 postfix_tensors=None,
...
class ColoredLoggingTensorHook(tf.estimator.LoggingTensorHook):
    ...
    def _log_tensors(self, tensor_values):
        ...
        if self._formatter:
            if elapsed_secs is not None:
                tf.compat.v1.logging.info("%s (%.3f sec)", self._formatter(
            ...
            if elapsed_secs is not None:
                tf.compat.v1.logging.info("%s (%.3f sec)", ", ".join(stats), elapsed_secs)
            else:
                tf.compat.v1.logging.info("%s", ", ".join(stats))
...
class SaveEvaluationResultHook(tf.estimator.SessionRunHook):
...
label_rank = tf.reshape(tf.where(tf.equal(top_classes, tf.expand_dims(labels, -1)))[:,-1], tf.shape(labels))
    ...
    def end(self, session):
        tf.compat.v1.logging.info('Evaluation results saved to %s', self._output_dir)
...
def dot_attention(inputs, memory, mask, units,
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        d_inputs = tf.compat.v1.layers.dropout(
            inputs=inputs,
            rate=drop_inputs,  # 0
        ...
        d_memory = tf.compat.v1.layers.dropout(
            inputs=memory,
            rate=drop_memory,  # 0
            noise_shape=None,  # [batch_size, 1, embed_dim]
            training=is_train,
        ...
        sequence_length = tf.shape(input=inputs)[1] # sequence_length

        inputs_ = tf.compat.v1.layers.dense(
            inputs=d_inputs,
            units=units,
        ...
        memory_ = tf.compat.v1.layers.dense(
            inputs=d_memory,
            units=units,
            activation=tf.nn.relu,
            use_bias=False,
        ...
        outputs = tf.matmul(inputs_, tf.transpose(a=memory_, perm=[0, 2, 1])) / (units ** 0.5)
        # mask
        ...
        with tf.compat.v1.variable_scope("gate"):
            ...
            d_res = tf.compat.v1.layers.dropout(
                inputs=res,
                rate=drop_res,  # 0
                noise_shape=None,  # [batch_size, 1, embed_dim]
                training=is_train,
...
def attention_image_summary(attn, image_shapes=None):
  ...
  image = tf.transpose(a=attn, perm=[0, 2, 3, 1])
  image = tf.pow(image, 0.2)  # for high-dynamic-range
  # Each head will correspond to one of RGB.
  # pad the heads to be a multiple of 3
  image = tf.pad(tensor=image, paddings=[[0, 0], [0, 0], [0, 0], [0, tf.math.mod(-num_heads, 3)]])
  image = split_last_dimension(image, 3)
  image = tf.reduce_max(input_tensor=image, axis=4)
  if image_shapes is not None:
    if len(image_shapes) == 4:
      ...
      image = tf.transpose(a=image, perm=[0, 1, 3, 2, 4, 5])
      ...
      image = tf.transpose(a=image, perm=[0, 1, 4, 3, 2, 5, 6, 7])
      image = tf.reshape(
          image,
...
def embedding_to_padding(emb):
  ...
  """
  emb_sum = tf.reduce_sum(input_tensor=tf.abs(emb), axis=-1)
...
def split_heads(x, num_heads):
  ...
  """
  return tf.transpose(a=split_last_dimension(x, num_heads), perm=[0, 2, 1, 3])
...
def compute_attention_component(antecedent,
  ...
  if filter_width == 1:
    ...
    return tf.nn.conv1d(
        input=antecedent, filters=total_depth, stride=filter_width, padding=padding, name=name)
...
def local_attention_1d(q, k, v, block_length=128, filter_width=100, name=None):
  ...
  with tf.compat.v1.variable_scope(
    ...
    def pad_to_multiple(x, pad_length):
      ...
      return tf.pad(tensor=x, paddings=[[0, 0], [0, 0], [0, -x_length % pad_length], [0, 0]])

    def pad_l_and_r(x, pad_length):
      return tf.pad(tensor=x, paddings=[[0, 0], [0, 0], [pad_length, pad_length], [0, 0]])
    ...
    k_t = tf.transpose(a=k, perm=[2, 0, 1, 3])
    k_new = tf.gather(k_t, gather_indices)
    k_new = tf.transpose(a=k_new, perm=[2, 3, 0, 1, 4])

    attention_bias = tf.expand_dims(embedding_to_padding(k_new) * -1e9, axis=-2)

    v_t = tf.transpose(a=v, perm=[2, 0, 1, 3])
    v_new = tf.gather(v_t, gather_indices)
    v_new = tf.transpose(a=v_new, perm=[2, 3, 0, 1, 4])
    ...
    if cache is not None:
      ...
      if memory_antecedent is not None:
        ...
        if decode_loop_step is None:
          ...
          tmp_k = tf.transpose(a=cache["k"], perm=[2, 0, 1, 3])
          tmp_k = inplace_ops.alias_inplace_update(
              tmp_k, decode_loop_step, tf.squeeze(k, axis=2))
          k = cache["k"] = tf.transpose(a=tmp_k, perm=[1, 2, 0, 3])
          tmp_v = tf.transpose(a=cache["v"], perm=[2, 0, 1, 3])
          tmp_v = inplace_ops.alias_inplace_update(
              tmp_v, decode_loop_step, tf.squeeze(v, axis=2))
          v = cache["v"] = tf.transpose(a=tmp_v, perm=[1, 2, 0, 3])
...
def layer_norm(x, filters=None, epsilon=1e-6, name=None, reuse=None):
  ...
  with tf.compat.v1.variable_scope(name, default_name="layer_norm", values=[x], reuse=reuse):
    scale = tf.compat.v1.get_variable("scale", [filters], initializer=tf.compat.v1.ones_initializer())
    bias = tf.compat.v1.get_variable("bias", [filters], initializer=tf.compat.v1.zeros_initializer())
...
def group_norm(x, filters=None, num_groups=8, epsilon=1e-5, name=None):
  ...
  # Prepare variables.
  with tf.compat.v1.variable_scope(name, default_name="group_norm", values=[x]):
    scale = tf.compat.v1.get_variable(
        "scale", [filters], initializer=tf.compat.v1.ones_initializer())
    bias = tf.compat.v1.get_variable(
        "bias", [filters], initializer=tf.compat.v1.zeros_initializer())
    epsilon, scale, bias = [cast_like(t, x) for t in [epsilon, scale, bias]]
    # Reshape and compute group norm.
    x = tf.reshape(x, x_shape[:-1] + [num_groups, filters // num_groups])
    # Calculate mean and variance on heights, width, channels (not groups).
    mean, variance = tf.nn.moments(x=x, axes=[1, 2, 4], keepdims=True)
    norm_x = (x - mean) * tf.math.rsqrt(variance + epsilon)
    return tf.reshape(norm_x, x_shape) * scale + bias

def noam_norm(x, epsilon=1.0, name=None):
  """One version of layer normalization."""
  with tf.compat.v1.variable_scope(name, default_name="noam_norm", values=[x]):
    shape = x.get_shape()
    ndims = len(shape)
...
def l2_layer_norm(x, filters=None, epsilon=1e-6, name=None, reuse=None):
  ...
  if filters is None:
    filters = shape_list(x)[-1]
  with tf.compat.v1.variable_scope(name, default_name="l2_layer_norm", values=[x], reuse=reuse):
    scale = tf.compat.v1.get_variable(
        "scale", [filters], initializer=tf.compat.v1.ones_initializer())
    bias = tf.compat.v1.get_variable(
        "bias", [filters], initializer=tf.compat.v1.zeros_initializer())
    epsilon, scale, bias = [cast_like(t, x) for t in [epsilon, scale, bias]]
    mean = tf.reduce_mean(input_tensor=x, axis=[-1], keepdims=True)
    l2norm = tf.reduce_sum(input_tensor=tf.square(x - mean), axis=[-1], keepdims=True)
...
def _norm(g, b, inp, scope, center=True):
  ...
  gamma_init = tf.compat.v1.constant_initializer(g)
  beta_init = tf.compat.v1.constant_initializer(b)
  with tf.compat.v1.variable_scope(scope):
    tf.compat.v1.get_variable("gamma", shape=shape, initializer=gamma_init)
    if center:
      tf.compat.v1.get_variable("beta", shape=shape, initializer=beta_init)
  normalized = tf.contrib.layers.layer_norm(inp, center=center, reuse=True, scope=scope)
  return normalized

class LayerNormLSTMCellv2(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def call(self, inputs, state):
    ...
    scope = tf.compat.v1.get_variable_scope()
    with tf.compat.v1.variable_scope(scope, initializer=self._initializer) as unit_scope:
      ...
      if self._use_peepholes:
        with tf.compat.v1.variable_scope(unit_scope):
...
def model_fn(features, labels, mode, params, config):
    ...
    # Embedding layer
    with tf.compat.v1.variable_scope('embedding_1', values=[features]):
        ...
        dropped_embedded = tf.compat.v1.layers.dropout(
            inputs=embedded,
    ...
    with tf.compat.v1.variable_scope('conv_1'):
        if params.use_conv_1_prenet:
            ...
            for d, units in enumerate(conv_1_prenet_sizes):
                with tf.compat.v1.variable_scope('prenet_{}'.format(d)):
        ...
        if params.use_conv_1_bank:
                ...
                if params.use_conv_1_bank_odd:
                    conv_fn = lambda k: \
                        tf.compat.v1.layers.conv1d(
                            ...
                            bias_initializer=tf.compat.v1.zeros_initializer(dtype=float_type),
                            trainable=True,
            ...
            convolved = tf.compat.v1.layers.conv1d(
                inputs=dropped_embedded,
                filters=params.conv_1_filters,  # 32
            ...
            # tf.Tensor: shape=(batch_size, sequence_length, conv_1_filters), dtype=float32
        ...
        convolved = tf.compat.v1.layers.dropout(
            inputs=convolved,
        ...
        if params.use_conv_1_highway: # True
            ...
            if convolved.shape[2] != params.conv_1_highway_units: # 32
                convolved = tf.compat.v1.layers.dense(convolved, params.conv_1_highway_units)
            for d in range(params.conv_1_highway_depth): # 3
                with tf.compat.v1.variable_scope('highway_{}'.format(d)):
    ...
    # bidirectional gru
    with tf.compat.v1.variable_scope('bi_rnn_1'):
        ...
        if rnn_cell_type.startswith('cudnn'):
            transposed_convolved = tf.transpose(
                a=convolved, perm=[1, 0, 2], name='transpose_to_rnn')
            lstm = cell(
            ...
            outputs = tf.transpose(
                a=outputs, perm=[1, 0, 2], name='transpose_from_rnn')
            ...
            outputs, output_states = tf.compat.v1.nn.bidirectional_dynamic_rnn(
    ...
    with tf.compat.v1.variable_scope('rnn_attention_1'):
    ...
    if mode != tf.estimator.ModeKeys.PREDICT:
        with tf.compat.v1.variable_scope('loss'):
            if params.use_crf:
                # crf layer
                with tf.compat.v1.variable_scope('crf_1'):
                ...
                loss = tf.reduce_sum(input_tensor=masked_losses) / \
                    tf.cast(tf.reduce_sum(input_tensor=lengths), dtype=tf.float32)
    ...
    with tf.compat.v1.variable_scope('evaluation'):
        ...
        # tf.enable_eager_execution()
        # batch, length, depth = (2, 6, 3)
        # all_probs = tf.random.uniform([batch, length, depth])
        # labels = tf.random.uniform([batch, length], maxval=3, dtype=tf.int32)
        # label_prob = tf.gather(all_probs, tf.expand_dims(labels, -1), batch_dims=-1)
        label_prob = tf.compat.v1.batch_gather(all_probs, tf.expand_dims(labels, -1))
        # fast_rank = tf.where(tf.equal(tf.nn.top_k(all_probs, 50)[1], tf.expand_dims(labels, -1)))[:,-1]
        with tf.device('/cpu:0'):
            # TODO: Optimize by trying top 100 first, then 1000, then all
            # label_rank = tf.cond(
            #     pred=tf.equal(tf.size(fast_rank), tf.size(labels)),
            #     true_fn=lambda: tf.reshape(fast_rank, tf.shape(labels)),
            #     false_fn=lambda: tf.reshape(tf.where(tf.equal(tf.contrib.framework.argsort(all_probs, direction='DESCENDING'), tf.expand_dims(labels, -1)))[:,-1], tf.shape(labels))
            # )
            label_rank = tf.reshape(tf.compat.v1.where(tf.equal(tf.argsort(all_probs, direction='DESCENDING'), tf.expand_dims(labels, -1)))[:,-1], tf.shape(input=labels))
    ...
    with tf.compat.v1.variable_scope('metrics'):
        ...
        with tf.compat.v1.name_scope('batch_accuracy', values=[predictions['classes'], labels]):
            is_correct = tf.cast(
            ...
            batch_accuracy = tf.compat.v1.div(tf.reduce_sum(
                input_tensor=is_correct), tf.reduce_sum(input_tensor=num_values))
        tf.compat.v1.summary.scalar('accuracy', batch_accuracy)
    ...
    optimizers = {
        ...
        'adam': lambda lr: tf.compat.v1.train.AdamOptimizer(lr, epsilon=params.adam_epsilon),
        'nadam': lambda lr: tf.contrib.opt.NadamOptimizer(lr, epsilon=params.adam_epsilon),
        'ftrl': tf.compat.v1.train.FtrlOptimizer,
        'momentum': lambda lr: tf.compat.v1.train.MomentumOptimizer(lr, momentum=0.9),
    ...
    with tf.compat.v1.variable_scope('optimizer'):
        ...
        def learning_rate_warmup(global_step, warmup_steps, repeat_steps=0, start=0.01, warmup_schedule='exp'):
            ...
            if warmup_schedule == 'exp':
                warmup = tf.exp(tf.math.log(start) / warmup_steps)**(warmup_steps - local_step)
            else:
                assert warmup_schedule == 'linear'
                warmup = ((tf.constant(1.) - start) / warmup_steps) * local_step + start
            return tf.compat.v1.where(local_step < warmup_steps, warmup, tf.constant(1.))
        ...
        if decay == 'noisy_linear_cosine_decay':
            lr = tf.compat.v1.train.noisy_linear_cosine_decay(
    ...
    examples_processed = tf.compat.v1.get_variable(
    ...
    group_inputs.append(tf.compat.v1.assign_add(examples_processed,
    ...
    if params.debug:
        train_op = tf.cond(
            pred=tf.logical_or(
                tf.math.is_nan(tf.reduce_max(input_tensor=embeddings)),
                tf.equal(global_step, 193000)
            ),
            false_fn=lambda: train_op,
            true_fn=lambda: tf.compat.v1.Print(train_op,
    ...
    training_hooks.append(tf.estimator.StepCounterHook(
    ...
    if params.trace:
        training_hooks.append(tf.estimator.ProfilerHook(
    ...
    if params.eval_level == 'topk':
        ...
        eval_tensors['label_rank'] = label_rank
        eval_tensors['top_classes'] = top_classes
        eval_tensors['top_probs'] = top_probs
...
def main(unused_args):
    ...
    if not FLAGS.num_classes or FLAGS.num_classes < 1:
        FLAGS.num_classes = FLAGS.metadata['domain_count']
    ...
    if FLAGS.num_gpus > 1:
        ...
        tf.compat.v1.logging.info('MirroredStrategy num_gpus: {}'.format(FLAGS.num_gpus))
    ...
    if FLAGS.use_jit_xla:
        session_config.graph_options.optimizer_options.global_jit_level = tf.compat.v1.OptimizerOptions.ON_1 # pylint: disable=no-member
    ...
    if FLAGS.job == 'eval':
        if not FLAGS.eval_checkpoint:
            FLAGS.eval_checkpoint = tf.train.latest_checkpoint(FLAGS.model_dir)
        tf.compat.v1.logging.info('Evaluating checkpoint: %s', FLAGS.eval_checkpoint)
        ...
        tf.compat.v1.logging.info('Exporting checkpoint: %s', FLAGS.export_checkpoint)
        ...
        tf.compat.v1.logging.info('Checkpoint exported to: %s', export_dir)
