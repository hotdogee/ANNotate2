...
def load_data(
    ...
    **kwargs
    ...
    if seq_regions_dict_cache_path.exists():
        print('Loading Sequence Regions from CACHE: {0}'.format('/'.join(seq_regions_dict_cache_path.parts[-2:])), end="", flush=True)
        start = time.perf_counter()
    ...
    if seq_ids_cache_path.exists():
        print('Loading Sequence Ids from CACHE: {0}'.format('/'.join(seq_ids_cache_path.parts[-2:])), end="", flush=True)
        start = time.perf_counter()
            ...
            if test_count < 0 or train_count < 0:
                print(
                    ...
                    .format(
                        ...
                        len(test_set & all_seq_set), len(train_set & all_seq_set)
                    )
                )
        ...
        for k in dataset:
            print(
                'Generating Domain Sequence Representation for {} Dataset'.format(
                    k.upper()
                )
            )
            ...
            metadata[k]['seq_count']['per_domain']['median'] = seq_count_per_domain[
                len(seq_count_per_domain) // 2]
...
def save_to_tfrecord(seq_ids, path, metadata, seq_dict, seq_regions_dict, writer_id=0):
    ...
    for k in dataset:
        ...
        for i in range(args.writers):
            op = Process(
                ...
                args=(
                    seq_ids
                    [int(i * seq_per_file):int((i + 1) * seq_per_file)],
                    os.path.join(
                        dataset_dir, f'{file_prefix}-{k}-{count_fmt.format(i+1)}.tfrecords'
