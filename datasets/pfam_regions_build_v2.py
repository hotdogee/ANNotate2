...
def save_to_tfrecord(data, path, metadata, seq_dict, seq_regions_dict):
    ...
    with tf.python_io.TFRecordWriter(path) as writer:
        ...
        for index, seq_id in enumerate(data['seq_ids']):
            prog.update(index)
            protein = np.array([aa_index[a] for a in seq_dict[seq_id]], dtype=np.uint8)
            domain = [domain_index['NO_DOMAIN']] * len(seq_dict[seq_id])
            for pfamA_acc, seq_start, seq_end in seq_regions_dict[seq_id]:
                domain = domain[:seq_start-1] + [domain_index[pfamA_acc]] * (seq_end - seq_start + 1) + domain[seq_end:]
...
if __name__ == "__main__":
    ...
    for k in dataset:
        ...
        save_to_tfrecord(dataset[k], os.path.join(dataset_dir,
            '{}-{}.tfrecords'.format(file_prefix, k)), metadata, seq_dict, seq_regions_dict)
