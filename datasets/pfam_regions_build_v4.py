...
def _pfam_regions_tsv_gz_to_dict(tsv_path):
    ...
    with gzip.open(tsv_path, 'r') as tsv_f:
        for line in tsv_f:
            ...
            domain_regions_dict[tokens[4]].append(
                (seq_id, int(tokens[5]), int(tokens[6]))
            )
        prog.update(tsv_f.tell())
    ...
    **kwargs
    ...
    # Arguments
        ...
        default_domain_list: If empty list, will build new domain list. If given existing
            ...
            default: []
        file_prefix: file name prefix for cache files.
            default: 'pfam-regions'
    ...
    uniprot_path = tf.keras.utils.get_file(
        uniprot_file,
        origin_base + uniprot_file,
        cache_subdir=cache_subdir,
        cache_dir=cache_dir
    )

    # ~/.keras/datasets/Pfam-A.regions.uniprot.tsv.gz
    regions_path = tf.keras.utils.get_file(
        regions_file,
        origin_base + regions_file,
        cache_subdir=cache_subdir,
        cache_dir=cache_dir
    )
    ...
    seq_dict_cache_path = '{0}.msgpack'.format(
        os.path.splitext(uniprot_path)[0]
    )
    ...
    print(
        'Loaded {:,} Sequences, Amino Acids Min {:,}, Median {:,}, Max {:,}, Total {:,}'
        .format(
            len(seq_lengths),
            seq_lengths[0], seq_lengths[len(seq_lengths) // 2], seq_lengths[-1],
            sum(seq_lengths)
        )
    )
    ...
    cache_subdir_path = Path(os.path.abspath(os.path.expanduser(cache_subdir)))
    seq_regions_dict_cache_path = cache_subdir_path / f'{file_prefix}-seq_regions.msgpack'
    if seq_regions_dict_cache_path.exists():
        print(
            'Loading Sequence Regions from CACHE: {0}'.format(
                '/'.join(seq_regions_dict_cache_path.parts[-2:])
            ),
            end="",
            flush=True
        )
        start = time.perf_counter()
        with seq_regions_dict_cache_path.open(mode='rb') as f:
            seq_regions_dict, domain_seq_dict = msgpack.load(f, raw=False)
        print(' - {:.0f}s'.format(time.perf_counter() - start))
    else:
        print('Loading Domain Region Data from ', end="", flush=True)
        # domain_regions_dict[pfamA_acc] = [(uniprot_acc + '.' + seq_version, seq_start, seq_end), ...]
        domain_regions_dict_cache_path = '{0}.msgpack'.format(
            os.path.splitext(regions_path)[0]
        )
        if os.path.exists(domain_regions_dict_cache_path):
            print(
                'CACHE: {0}'.format(domain_regions_dict_cache_path),
                end="",
                flush=True
            )
            start = time.perf_counter()
        ...
        print(
            'Loaded {:,} Domains, Regions Min {:,}, Median {:,}, Max {:,}, Total {:,}'
            .format(
                len(domain_lengths), domain_lengths[0],
                domain_lengths[len(domain_lengths) // 2], domain_lengths[-1],
                sum(domain_lengths)
            )
        )
        ...
        print(
            'Collecting Sequences Containing the Top {} Domains'.
            format(num_domains_wanted)
        )
        prog = tf.keras.utils.Progbar(num_domains_wanted)
        for i, pfamA_acc in enumerate(
            sorted(
                domain_regions_dict,
                key=lambda k: (len(domain_regions_dict[k]), k),
                reverse=True
            )
        ):
            ...
            for seq_id, seq_start, seq_end in domain_regions_dict[pfamA_acc]:
                ...
                if seq_id not in domain_seq_dict[pfamA_acc]:
                    domain_seq_dict[pfamA_acc][seq_id] = 1
        prog.update(num_domains_wanted)
        del domain_regions_dict
        with seq_regions_dict_cache_path.open(mode='wb') as f:
            msgpack.dump((seq_regions_dict, domain_seq_dict), f)
            # uniprot.msgpack 23.1 GB (24,875,252,089 位元組)
    print(
        'Collected {:,} Sequences with {:,} Domains'.format(
            len(seq_regions_dict), len(domain_seq_dict)
        )
    )
    # 16712/16712 [==============================] - 140s 8ms/step
    # Collected 54,223,493 Sequences with 16,712 Domains

    seq_ids_cache_path = cache_subdir_path / f'{file_prefix}-seq_ids.msgpack'
    if seq_ids_cache_path.exists():
        print(
            'Loading Sequence Ids from CACHE: {0}'.format(
                '/'.join(seq_ids_cache_path.parts[-2:])
            ),
            end="",
            flush=True
        )
        start = time.perf_counter()
        with seq_ids_cache_path.open(mode='rb') as f:
            dataset, metadata = msgpack.load(f, raw=False)
        print(' - {:.0f}s'.format(time.perf_counter() - start))
    else:
        ...
        test_set = set()
        np.random.seed(seed)
        status_text = 'Distributing Sequences to Training and Testing Sets with {:.0%} Split'.format(
            test_split
        )
        if max_seq_per_class_in_train:
            status_text += ' and Max {} Sequences per Domain in Training Set'.format(
                max_seq_per_class_in_train
            )
        if max_seq_per_class_in_test:
            status_text += ' and Max {} Sequences per Domain in Testing Set'.format(
                max_seq_per_class_in_test
            )
        print(status_text)
        prog = tf.keras.utils.Progbar(len(domain_seq_dict))
        # assign domains with fewer sequences first
        for i, pfamA_acc in enumerate(
            sorted(domain_seq_dict, key=lambda k: (len(domain_seq_dict[k]), k))
        ):
            prog.update(i)
            ...
            all_seq_set = set(domain_seq_dict[pfamA_acc])  # set of keys
            ...
            if test_count < 0 or train_count < 0:
                print(
                    '\r WARNING: {}={}, seq_count={}, test_count={}, train_count={}, already in test_set= {}, already in train_set= {}'
                    .format(
                        i, pfamA_acc, seq_count, test_count, train_count,
                        len(test_set & all_seq_set),
                        len(train_set & all_seq_set)
                    )
                )
        ...
        print(
            'Collected {:,} Training Sequences and {:,} Testing Sequences with {:,} Domains'
            .format(len(train_set), len(test_set), len(domain_list))
        )
        del domain_seq_dict
        ...
        for k in dataset:
            print(
                'Generating Domain Sequence Representation for {} Dataset'.
                format(k.upper())
            )
            if len(dataset[k]['seq_ids']) == 0:
                print(
                    'No sequences to process for {} Dataset'.format(k.upper())
                )
                continue
            ...
            # record metadata
            seq_count_per_domain = sorted(
                [len(s) for s in domain_seq_count.values()]
            )
            metadata[k]['seq_count']['per_domain']['min'
                                                  ] = seq_count_per_domain[0]
            metadata[k]['seq_count']['per_domain'][
                'median'] = seq_count_per_domain[len(seq_count_per_domain) // 2]
            metadata[k]['seq_count']['per_domain']['max'
                                                  ] = seq_count_per_domain[-1]
        ...
        with seq_ids_cache_path.open(mode='wb') as f:
            msgpack.dump((dataset, metadata), f)

    return dataset, metadata, seq_dict, seq_regions_dict
...
def save_to_tfrecord(
    seq_ids, path, metadata, seq_dict, seq_regions_dict, writer_id=0
):
    ...
    aa_index = metadata['aa_index']
    domain_index = metadata['domain_index']
    with tf.python_io.TFRecordWriter(path) as writer:
        if writer_id == 0:
            prog = tf.keras.utils.Progbar(len(seq_ids))
        for index, seq_id in enumerate(seq_ids):
            if writer_id == 0:
                prog.update(index)
            protein = np.array(
                [aa_index[a] for a in seq_dict[seq_id]], dtype=np.uint8
            )
            domain = [domain_index['NO_DOMAIN']] * len(seq_dict[seq_id])
            for pfamA_acc, seq_start, seq_end in seq_regions_dict[seq_id]:
                domain = domain[:seq_start - 1] + [
                    domain_index[pfamA_acc]
                ] * (seq_end - seq_start + 1) + domain[seq_end:]
            domains = np.array(domain, dtype=np.uint16)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        'Include only the top N domain classes in the dataset file, include all domain classes if None.'
    )
    parser.add_argument(
        '-s',
        '--test_split',
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        'Maximum the number of sequences to include in the training datasets, default is no limit.'
    )
    parser.add_argument(
        '-e',
        '--max_seq_per_class_in_test',
        ...
        default=None,
        help=
        'Maximum the number of sequences to include in the testing datasets, default is no limit.'
    )
    parser.add_argument(
        '-d',
        '--dataset_dir',
    ...
    parser.add_argument(
        ...
        default=4,
        help=
        'Number of output workers, will create the same number of tfrecord files. (default: %(default)s)'
    )
    ...
    file_prefix = 'pfam-regions-d{}-s{}'.format(
        args.num_classes or 0, int(args.test_split * 100)
    )
    if args.max_seq_per_class_in_train:
        file_prefix = '{}-t{}'.format(
            file_prefix, args.max_seq_per_class_in_train
        )
    if args.max_seq_per_class_in_test:
        file_prefix = '{}-e{}'.format(
            file_prefix, args.max_seq_per_class_in_test
        )
    print('\nPreparing Dataset: {}\n'.format(file_prefix))
    ...
    dataset, metadata, seq_dict, seq_regions_dict = load_data(
        origin_base=
        f'ftp://ftp.ebi.ac.uk/pub/databases/Pfam/releases/Pfam{args.pfam_version}/',
        num_domain=args.num_classes,
    ...
    for k in dataset:
        seq_ids = dataset[k]['seq_ids']
        seq_count = len(seq_ids)
        if seq_count == 0:
            print('No sequences to save for {} Dataset'.format(k.upper()))
            continue
        seq_per_file = seq_count / args.writers
        count_fmt = "{0:0" + str(len(str(args.writers))) + "d}"
        # output process
        output_processes = []
        for i in range(args.writers):
            op = Process(
                target=save_to_tfrecord,
                args=(
                    seq_ids[int(i * seq_per_file):int((i + 1) * seq_per_file)],
                    os.path.join(
                        dataset_dir,
                        f'{file_prefix}-{k}-{count_fmt.format(i+1)}.tfrecords'
                    ), metadata, seq_dict, seq_regions_dict, i
                )
            )
            op.start()
