...
if __name__ == "__main__":
    ...
    print(f'GOs: {len(go_count)}')
    index_from = 1
    aa_list = 'FLIMVPAWGSTYQNCO*UHKRDEBZX-'
    aa_index = dict(zip(aa_list, range(index_from, index_from + len(aa_list))))
    go_list = ['PAD', 'START', 'END'] + sorted(go_count, key=lambda k: (go_count[k], k), reverse=True)
                ...
                'seq_count':
                    {
                        ...
                        'per_class':
                            {
                                'min': go_count[go_list[-1]],
                                'median': go_count[go_list[len(go_list) // 2]],
                                'max': go_count[go_list[0]]
                            }
                    },
                'seq_len':
                    {
    ...
    with tf.python_io.TFRecordWriter(str(train_path)) as writer, tqdm(
        ...
        for seq_id in train_list:
            ...
            go = np.array([1] + sorted([go_index[go_id] for go_id in seq_gos[seq_id]]) + [2], dtype=np.uint16)
...
# W2125
# python datasets/go_tfrecords_build_v2.py --gaf1 /data12/goa/goa-20191015/goa_uniprot_all.gaf --fa2 /data12/uniprot/uniprot-20191015/uniprot_trembl.fasta --fa3 /data12/uniprot/uniprot-20191015/uniprot_sprot.fasta --fa4 /data12/uniprot/uniprot-20191015/uniprot_sprot_varsplic.fasta --meta /data12/goa/goa-20191015/goa-20191015-v2-meta.json --train /data12/goa/goa-20191015/goa-20191015-v2-train.tfrecords
